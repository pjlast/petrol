...
and (_, !'res) query =
  ...
  | TABLE : { table : table_name } -> ('a, [> `TABLE ]) query
...
(* let () = pp_fn := { pp_expr = fun (type a) fmt (expr: a expr) : unit ->
   pp_expr fmt expr } *)
let add_printer f =
  let old_pp = !pp_fn in
  pp_fn := f old_pp
...
let rec ty_expr_list : 'a. 'a expr_list -> 'a Type.ty_list =
 fun (type a) (ls : a expr_list) : a Type.ty_list ->
  match ls with
  | [] -> Nil
  | h :: t -> Cons (ty_expr h, ty_expr_list t)
...
and pp_join : int -> Format.formatter -> join -> unit =
 fun n fmt (MkJoin { table; on; join_op }) ->
  match table with
  | TABLE { table } ->
      Format.fprintf fmt "%a %s ON %a" pp_join_op join_op (snd table) pp_expr on
  | _ ->
      Format.fprintf fmt "%a (%a) AS join_tmp_%d ON %a" pp_join_op join_op
        pp_query table n pp_expr on
...
and query_values :
 ...
 fun acc (type a b) (query : (a, b) query) ->
  ...
  | INSERT { table = _; on_err = _; on_conflict = _; set; returning } ->
      ...
      acc
  | TABLE _ ->
      (* Since TABLE adds no new values (only created on joins), we simply
